...
awk '{print $4}' $directory/failed_login_collective.txt | sort | uniq -c| awk '{print "data.addRow([\x27"$2"\x27, "$1"]);"}'|cat > javascript_temp.txt

##this is all then passed to the wrap_contents.sh file that was created in the prelab
./bin/wrap_contents.sh javascript.txt ./html_components/username_dist username_dist.html $directory 

#the temporaru files are then removed. 
rm javascript_temp.txt
rm $directory/failed_login_collective.txt
